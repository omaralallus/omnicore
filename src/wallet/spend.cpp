...
std::optional<SelectionResult> ChooseSelectionResult(const CWallet& wallet, const CAmount& nTargetValue, const CoinEligibilityFilter& eligibility_filter, const std::vector<COutput>& available_coins, const CoinSelectionParams& coin_selection_params)
{
    ...
    if (coin_selection_params.m_select_bnb) {
...
static util::Result<CreatedTransactionResult> CreateTransactionInternal(
        ...
        bool sign,
        bool omni,
        CAmount* required_fee) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    AssertLockHeld(wallet.cs_wallet);
    ...
    for (const auto& recipient : vecSend) {
        ...
        if (recipient.fSubtractFeeFromAmount) {
            if (coin_control.m_subtract_fee_from_change) {
                return util::Error{_("Subtract from change and recipient are both set")};
            }
    ...
    // Get available coins
    const auto available_coins = AvailableCoins(wallet,
                                                &coin_control,
    ...
    // Choose coins to use
    auto available_coins_copy = available_coins;
    std::optional<SelectionResult> result = SelectCoins(wallet, available_coins_copy, /*nTargetValue=*/selection_target, coin_control, coin_selection_params);
    if (!result) {
    ...
    if (omni) {
        ...
        auto amount = result->GetChange(0, coin_selection_params.m_change_fee);
        CAmount dust = GetDustThreshold(CTxOut(amount, scriptChange), coin_selection_params.m_discard_feerate);
        while (amount < dust) {
            available_coins_copy = available_coins;
            coin_selection_params.m_select_bnb = false;
            auto value_to_select = selection_target + (dust - amount);
            result = SelectCoins(wallet, available_coins_copy, value_to_select, coin_control, coin_selection_params);
            if (!result) {
                return util::Error{_("Insufficient funds")};
            }
            amount = result->GetChange(0, coin_selection_params.m_change_fee);
        }
    }
    ...
    fee_needed = std::max(coin_control.m_mininum_fee, fee_needed);
    ...
    if (coin_control.m_subtract_fee_from_change && fee_needed > nFeeRet) {
        if (nChangePosInOut == -1) {
            return util::Error{_("No change output to pay the fee")};
        }
        auto& change = txNew.vout.at(nChangePosInOut);
        change.nValue -= (fee_needed - nFeeRet);
        if (required_fee) {
            auto dust = GetDustThreshold(change, wallet.chain().relayDustFee());
            *required_fee = fee_needed - nFeeRet + (change.nValue > dust ? 0 : dust);
        }
        // Error if this output is reduced to be below dust
        if (IsDust(change, wallet.chain().relayDustFee())) {
            if (change.nValue < 0) {
                return util::Error{_("The change amount is too small to pay the fee")};
            } else {
                return util::Error{_("The change amount is too small to send after the fee has been deducted")};
            }
        }
        nFeeRet = fee_needed;
    }
    ...
    if (coin_selection_params.m_subtract_fee_outputs && outputs_to_subtract_fee_from > 0) {
        ...
        CAmount* required_fee)
{
    ...
    auto res = CreateTransactionInternal(wallet, vecSend, change_pos, coin_control, sign, omni, required_fee);
    ...
    if (txr_ungrouped.fee > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        ...
        auto txr_grouped = CreateTransactionInternal(wallet, vecSend, change_pos, tmp_cc, sign, omni, required_fee);
