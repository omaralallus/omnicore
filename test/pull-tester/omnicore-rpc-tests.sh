...
if [ "$1" = "true" ]; then
    ...
    $OMNICORED -datadir="$DATADIR" -addresstype=p2sh-segwit -regtest -txindex -server -daemon -rpcuser=bitcoinrpc -rpcpassword=pass -debug=1 -omnidebug=all -omnialertallowsender=any -omniactivationallowsender=any -paytxfee=0.0001 -minrelaytxfee=0.00001 -limitancestorcount=750 -limitdescendantcount=750 -rpcserialversion=0 -discover=0 -listen=0 -acceptnonstdtxn=1 -deprecatedrpc=label -deprecatedrpc=labelspurpose &
else
    echo "Debug logging level: minimum"
    $OMNICORED -datadir="$DATADIR" -addresstype=p2sh-segwit -regtest -txindex -server -daemon -rpcuser=bitcoinrpc -rpcpassword=pass -debug=0 -omnidebug=none -omnialertallowsender=any -omniactivationallowsender=any -paytxfee=0.0001 -minrelaytxfee=0.00001 -limitancestorcount=750 -limitdescendantcount=750 -rpcserialversion=0 -discover=0 -listen=0 -acceptnonstdtxn=1 -deprecatedrpc=label -deprecatedrpc=labelspurpose &
fi
