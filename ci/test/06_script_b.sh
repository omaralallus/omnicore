...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  CI_EXEC "${TEST_RUNNER_ENV}" DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${BASE_OUTDIR}/build/omnicore-*/src/test/test_omnicore" --catch_system_errors=no -l test_suite
fi
...
if [ "$RUN_OMNIJ_TESTS" = "true" ]; then
  CI_EXEC source /tmp/cirrus-ci-build/test/pull-tester/omnicore-rpc-tests.sh
fi
